...
- (void)setModel:(TMBillModel *)model {
    _model = model;
    self.dayLabel.text = STR(model.date);
    self.tabLabel.text = STR(model.type);
    self.priceLabel.text = STR(model.pirce);
    self.dayLabel.hidden = self.totalLabel.hidden = self.dotImgView.hidden = !model.showDay;
}
